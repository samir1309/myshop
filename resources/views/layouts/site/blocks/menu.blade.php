				...
				<div class="row w-100 m-0">
					...
					<div class="col-lg-4 col-md-2 p-xxl-2 text-center p-md-1 align-self-center">
						<a href="{{route('site.home')}}" class="navbar-brand mx-auto">
							<img src="{{asset('assets/uploads/content/set/'.$setting_header->logo)}}" alt="{{$setting_header->title}}" class="text-white" width="35%" height="100%">
						</a>
					</div>
						...
						<ul class="p-0 my-0 ms-auto max-content user d-flex me-0">
							...
							<li class="d-inline my-0 ms-2 me-0 dr position-relative">
								<a @if(!\Auth::check()) href="{{route('panel.log')}}" @else
									...
									<div class="user-icon">
										<i class="bi bi-suit-heart d-flex"></i>
									</div>
									<div class="over-tool">
										<p class="m-0">
			...
			<div class="row w-100 m-0 menu-form-row">
				...
				<div class="col-sm-2 col-xs-3 align-self-center p-1">
					<div class="d-flex align-items-center justify-content-center">
						<a @if(\Auth::check()) href="{{route('site.cart.checkout')}}" @else href="{{route('panel.log')}}" @endif class="d-flex max-content mx-auto text-dark p-1 align-items-center justify-content-center">
							<i class="bi bi-cart2 d-flex fs-3"></i>
						</a>
						<a @if(!\Auth::check()) href="{{route('panel.log')}}" @else href="{{route('panel.favorites')}}" @endif class="d-flex max-content mx-auto text-dark p-1 align-items-center justify-content-center">
							<i class="bi bi-suit-heart d-flex fs-3"></i>
						</a>
					</div>
				</div>
			</div>
	...
	<div class="row w-100 m-0">
		<div class="col-xs text-center col-sm psd">
			<a href="{{route('site.home')}}" @if(count($seg)==0) class="active" @endif>
			    <i class="bi bi-house bi-inactive"></i>
            	<i class="bi bi-house-fill bi-active"></i>
				<p>
					خانه
				</p>
			</a>
		...
		<div class="col-xs text-center col-sm psd">
			<a href="" class="" data-bs-toggle="modal" data-bs-target="#catModal">
			    <i class="bi bi-grid bi-inactive"></i>
			    <i class="bi bi-grid-fill bi-active"></i>
				<p>
					دسته بندی
				</p>
			</a>
		...
		<div class="col-xs text-center col-sm psd">
            <a href="{{route('site.dis')}}" @if(count($seg)==1 && $seg[0]=='discount' ) class="active"
                @endif>
                <i class="bi bi-tags bi-inactive"></i>
		        <i class="bi bi-tags-fill bi-active"></i>
				<p>
			    	تخفیف دار ها
				</p>
			</a>
		...
		<div class="col-xs text-center col-sm psd">
			<a @if(!\Auth::check()) href="{{route('panel.log')}}" @else href="{{route('panel.dashboard')}}"
				...
				@endif>
				<i class="bi bi-person bi-inactive"></i>
            	<i class="bi bi-person-fill bi-active"></i>
				<p>
