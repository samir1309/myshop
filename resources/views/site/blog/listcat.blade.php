			...
			<div class="col-sm-6 col-lg-4 col-xl-3">
				<div class="card bg-transparent">
					<div class="overflow-hidden rounded-3">
						<img src="{{asset('assets/uploads/content/art/medium/'.$data->image)}}" class="card-img" alt="course image">
						<!-- Overlay -->
						<div class="bg-overlay bg-dark opacity-4"></div>
						<div class="card-img-overlay d-flex align-items-start p-3">
							<!-- badge -->
							<a href="{{ route('site.blog.cat', @$data->category->id ) }}" class="badge text-bg-{{ $data->show_homePage}}">{{ $data->category->title}}</a>
						</div>
					</div>
					...
					<div class="card-body">
						<!-- Title -->
						<h5 class="card-title fw-normal"><a href="{{ route('site.blog.details', @$data->url ) }}">{{ $data->title}}</a></h5>
						<!-- <p class="text-truncate-2">{!! strip_tags(\Illuminate\Support\Str::words(@$data->description,30)) !!}</p>
                        <div class="d-flex justify-content-between">
							<h6 class="mb-0 fw-normal"><a href="#">لیلا اسدی</a></h6>
							<span class="small"><i class="bi bi-eye text-one d-flex ms-2">{{$data->view}}</i></span>
						</div> -->
					</div>
				</div>
			</div>
